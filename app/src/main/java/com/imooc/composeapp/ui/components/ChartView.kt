...
fun ChartView( points: List<Double>,modifier: Modifier = Modifier) {
    ...
    var perY = 8 // 24*5/15 每8dp代表1积分，也就是每一行3积分
    ...
    Canvas(modifier = modifier.size(width = canvasWidth.dp, height = canvasHeight.dp)) {
        ...
        for (index in 0 until points.count()) {
            ...
            drawCircle(
                ...
                center = circleCenter, style = Stroke(width = 5f)
            )
            if (index < points.count() - 1) {
                // 下一个点的坐标:也就是circleCenter
                ...
                drawLine(
                    Color(0xFF149EE7),
                    start = circleCenter, end = nextPointOffset,
                    strokeWidth = 5f
                )
            }
        }
